...
class TestCRSRetrieval:
    ...
    def test_get_crs_by_session(self, client, db, client_token, sample_crs, setup_team_project, client_user):
        """Test getting CRS by session ID."""
        project = setup_team_project["project"]
    ...
    def test_get_crs_by_id(self, client, client_token, sample_crs):
        """Test getting CRS by ID."""
        response = client.get(
            f"/api/crs/{sample_crs.id}",
            headers={"Authorization": f"Bearer {client_token}"}
        )
    ...
    def test_get_crs_versions(self, client, db, client_token, sample_crs, setup_team_project, client_user):
        """Test getting CRS version history."""
        project = setup_team_project["project"]
        ...
        response = client.get(
            f"/api/crs/versions?project_id={project.id}",
            headers={"Authorization": f"Bearer {client_token}"}
        )
...
class TestCRSStatusUpdate:
    ...
    def test_approve_crs(self, client, db, client_token, sample_crs, client_user):
        """Test approving a CRS document."""
        # Update user to project manager role
        ...
        response = client.put(
            f"/api/crs/{sample_crs.id}/status",
            json=payload,
            headers={"Authorization": f"Bearer {client_token}"}
        )
    ...
    def test_reject_crs(self, client, db, client_token, sample_crs, client_user):
        """Test rejecting a CRS document."""
        # Update user to project manager role
        member = db.query(TeamMember).filter_by(user_id=client_user.id).first()
        member.role = "admin"
    ...
    def test_reject_without_reason(self, client, db, client_token, sample_crs, client_user):
        """Test rejecting CRS without providing reason."""
        member = db.query(TeamMember).filter_by(user_id=client_user.id).first()
        member.role = "admin"
        db.commit()
    ...
    def test_update_status_unauthorized_role(self, client, client_token, sample_crs):
        """Test status update by non-PM user."""
        payload = {"status": "approved"}
...
class TestCRSReview:
    ...
    def test_get_pending_reviews(self, client, db, client_token, sample_crs, client_user):
        ...
        # Update CRS to under_review
        sample_crs.status = CRSStatus.under_review
        db.commit()
        ...
        data = response.json()
        assert len(data) >= 1
        assert any(crs["id"] == sample_crs.id for crs in data)
    
    def test_get_my_crs_requests(self, client, client_token, sample_crs):
        """Test getting user's own CRS requests."""
        response = client.get(
            "/api/crs/my-requests",
            headers={"Authorization": f"Bearer {client_token}"}
...
class TestCRSAudit:
    ...
    def test_get_audit_logs(self, client, db, client_token, sample_crs, client_user):
        """Test getting CRS audit logs."""
        from app.models.audit import CRSAuditLog
...
class TestCRSPreview:
    ...
    @patch("app.api.crs.generate_preview_crs")
    def test_get_session_preview(self, mock_preview, client, db, client_token, setup_team_project, client_user):
        """Test getting CRS preview for a session."""
        project = setup_team_project["project"]
    ...
    @patch("app.api.crs.generate_preview_crs")
    def test_preview_with_pattern(self, mock_preview, client, db, client_token, setup_team_project, client_user):
        """Test preview with specific pattern."""
        project = setup_team_project["project"]
...
class TestCRSContentUpdate:
    ...
    def test_update_crs_content(self, client, db, client_token, sample_crs):
        """Test updating CRS content."""
        new_content = {
            "project_title": "Updated Title",
            "project_description": "Updated description"
        ...
        response = client.put(
            f"/api/crs/{sample_crs.id}/content",
            json=payload,
            headers={"Authorization": f"Bearer {client_token}"}
        )
        ...
        # Verify edit_version incremented
        db.refresh(sample_crs)
        assert sample_crs.edit_version == 2
    
    def test_update_approved_crs_creates_new_version(self, client, db, client_token, sample_crs, client_user):
        """Test that updating approved CRS creates new version."""
        # Set CRS to approved
        sample_crs.status = CRSStatus.approved
        sample_crs.approved_by = client_user.id
        db.commit()
