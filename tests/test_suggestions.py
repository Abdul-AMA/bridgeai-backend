...
class TestSuggestionsNode:
    ...
    def test_suggestions_node_success(self):
        ...
        with patch(
            "app.ai.nodes.suggestions.suggestions_node.generate_creative_suggestions"
        ) as mock_generate:
            ...
            # Mock memory search
            with patch(
                "app.ai.nodes.suggestions.suggestions_node.search_project_memories"
            ) as mock_search:
    ...
    def test_suggestions_node_missing_context(self):
        ...
        assert (
            "suggestions_generated" not in result or not result["suggestions_generated"]
        )
