...
from app.models import SessionModel as ChatSession
from app.models import (
    Team,
    TeamMember,
    User,
...
class TestIndexEffectiveness:
    ...
    def test_projects_team_index_used(self, db: Session, sample_teams):
        ...
        assert (
            "ix_projects_team_id" in explain_output or "range" in explain_output.lower()
    ...
    def test_messages_session_index_used(self, db: Session, sample_session):
        ...
        assert (
            "ALL" not in explain_output or "ref" in explain_output
...
class TestConnectionPoolStress:
    ...
    def test_connection_pool_configuration(self):
        ...
        assert hasattr(
            pool, "_max_overflow"
        ), "Pool should have max_overflow configured"
...
class TestN1QueryDetection:
    ...
    def test_projects_no_n1_queries(self, db: Session, sample_teams, sample_user):
        ...
        fetched_projects = (
            db.query(Project)
            .options(joinedload(Project.creator), joinedload(Project.team))
            .filter(Project.team_id.in_([t.id for t in sample_teams]))
            .all()
        )
    ...
    def test_notifications_batch_loading(self, db: Session, sample_user, sample_teams):
        ...
        fetched_notifs = (
            ...
            .filter(Notification.user_id == sample_user.id)
            .limit(30)
            .all()
        )
    ...
    def test_messages_session_query_efficiency(
...
class TestChromaDBPerformance:
    ...
    def test_search_with_server_side_filtering(self):
        ...
        results = chroma_manager.search_embeddings(
            query="document", project_id=2, n_results=10
        )

        # All results should be from project_id=2
        for result in results:
            assert (
                result["metadata"]["project_id"] == 2
    ...
    def test_search_empty_collection(self):
        ...
        results = chroma_manager.search_embeddings(
            query="non-existent", project_id=99999, n_results=5
        )
...
class TestQueryPerformanceUnderLoad:
    ...
    def test_projects_query_with_1000_projects(
        self, db: Session, sample_teams, sample_user
    ):
        ...
        start = time.time()
        team_projects = (
            db.query(Project).filter(Project.team_id == sample_teams[0].id).all()
        )
        query_time = (time.time() - start) * 1000
    ...
    def test_notifications_query_with_10000_notifications(
        ...
        unread = (
            db.query(Notification)
            .filter(
                Notification.user_id == sample_user.id, Notification.is_read == False
            )
        ...
        assert (
            query_time < 100
        ), f"Query too slow: {query_time:.2f}ms with 10k notifications"
        assert len(unread) > 0, "Should return unread notifications"

    def test_crs_documents_query_with_500_documents(
        self, db: Session, sample_project, sample_user
    ):
...
class TestMemoryLeaks:
    ...
    def test_session_cleanup_after_queries(self, db: Session):
        ...
        initial_sessions = len(
            [obj for obj in gc.get_objects() if isinstance(obj, Session)]
        )
...
class TestEdgeCases:
    ...
    def test_query_with_no_results(self, db: Session):
        ...
        projects = (
            db.query(Project)
            .filter(Project.team_id == 999999)  # Non-existent team
            .all()
        )
        query_time = (time.time() - start) * 1000
    ...
    def test_query_with_null_foreign_keys(self, db: Session, sample_user):
        ...
        try:
            ...
            projects = db.query(Project).filter(Project.team_id.is_(None)).all()
    ...
    def test_large_in_clause(self, db: Session, sample_teams):
        ...
        start = time.time()
        projects = db.query(Project).filter(Project.team_id.in_(large_team_ids)).all()
        query_time = (time.time() - start) * 1000
...
class TestPerformanceRegression:
    ...
    def test_write_performance_not_degraded(
        self, db: Session, sample_user, sample_team
    ):
...
def sample_teams(db: Session, sample_user):
    ...
    for i in range(5):
        ...
        # Add user as member
        member = TeamMember(
            team_id=team.id, user_id=sample_user.id, role="admin", is_active=True
        )
        db.add(member)
        teams.append(team)
